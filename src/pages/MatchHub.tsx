...
function FloatingEmoji({ emoji, onComplete }: { emoji: string; onComplete: () => void }) {
  ...
  return (
    <div className="fixed bottom-24 pointer-events-none animate-float-up text-4xl md:text-6xl z-[100] drop-shadow-2xl" style={{ left: `${left}%` }}>{emoji}</div>
  )
}
...
export default function MatchHubPage() {
  ...
  useEffect(() => {
    ...
    const channel = api.subscribeToMatchHub(matchId, (payload: any) => {
      if (payload.eventType === 'INSERT') {
        if (payload.new.type === 'REACT') setActiveReacts(prev => [...prev, { id: Date.now(), emoji: payload.new.emoji }])
        else loadData()
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col h-[calc(100dvh-100px)] md:h-[calc(100vh-120px)] animate-in fade-in duration-500 overflow-hidden relative rounded-2xl my-0 md:my-4 border border-slate-100 shadow-2xl bg-white">
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
                       {SUGGESTED_TAGS.map(tag => (
                         <button key={tag} onClick={() => { setGiphySearch(tag); fetchGiphy(tag, activeGiphyType); }} className="px-3 py-1.5 bg-slate-50 rounded-full text-[10px] font-bold text-slate-500 whitespace-nowrap hover:bg-[#0a2647] hover:text-white transition-all">#{tag}</button>
                       ))}
                    </div>
                    {loadingGiphy ? (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pb-10">
                         {giphyResults.map(g => (
                           <button key={g.id} onClick={() => handleSendMedia(g.images.fixed_height.url)} className="relative aspect-video overflow-hidden rounded-lg bg-slate-50 border border-slate-100 hover:border-[#0a2647] transition-all group">
                              <img src={g.images.fixed_height_small.url} className="w-full h-full object-cover transition-transform group-hover:scale-110" alt="gif" />
                           </button>
                         ))}
                      </div>
                    )}
      ...
      {/* COMPACT INPUT AREA WITH ANIMATIONS */}
      <footer className={`shrink-0 bg-white border-t border-slate-50 transition-all duration-500 ease-in-out ${isInputCollapsed ? 'p-2 pb-5' : 'p-3 md:p-5 pb-8'} relative z-[80]`}>
         ...
         <button
           ...
           className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-white border border-slate-100 text-slate-200 w-14 h-5 rounded-full flex items-center justify-center shadow-sm hover:text-[#0a2647] transition-colors"
         >
         ...
         <form onSubmit={handleSendMessage} className="flex gap-2 items-center">
           ...
           >
              <svg
                ...
                className={`transition-all duration-700 ease-in-out transform ${isInputCollapsed ? 'h-5 w-5 rotate-[360deg] scale-110' : 'h-6 w-6 rotate-0 scale-100'}`}
                viewBox="0 0 20 20"
                fill="currentColor"
              >
